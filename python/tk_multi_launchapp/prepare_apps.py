...
class PrepareAppsForLaunch(object):
    def __init__(self):
        self._app = sgtk.platform.current_bundle()

    def prepare_launch_for_engine(
        self, engine_name, context,
        file_to_open=None, app_path=None, app_args=None
    ):
        ...
        if engine_name == "tk-maya":
            self._prepare_maya_launch(app_path)
        elif engine_name == "tk-softimage":
            self._prepare_softimage_launch()
        elif engine_name == "tk-motionbuilder":
            app_args = self._prepare_motionbuilder_launch(app_args)
        elif engine_name == "tk-3dsmax":
            app_args = self._prepare_3dsmax_launch(app_args)
        elif engine_name == "tk-3dsmaxplus":
            app_args = self._prepare_3dsmaxplus_launch(context, app_args, app_path)
        elif engine_name == "tk-photoshop":
            self._prepare_photoshop_launch(context)
        elif engine_name == "tk-houdini":
            self._prepare_houdini_launch(context)
        elif engine_name == "tk-mari":
            self._prepare_mari_launch(engine_name, context)
        elif engine_name in ["tk-flame", "tk-flare"]:
            (app_path, app_args) = self._prepare_flame_flare_launch(
                engine_name,
                context,
                app_path,
                app_args,
            ...
            try:
                ...
                if engine_name == "tk-nuke":
                    app_args = self._prepare_nuke_launch(file_to_open, app_args)
                elif engine_name == "tk-hiero":
                    self._prepare_hiero_launch()
                else:
    ...
    def prepare_generic_launch(self, engine_name, context, app_path, app_args):
        ...
        engine_path = sgtk.platform.get_engine_path(engine_name, self._app.sgtk, context)
    ...
    def _prepare_nuke_launch(self, file_to_open, app_args):
        """
        Nuke specific pre-launch environment setup.
        """
        # Make sure Nuke can find the Tank menu
        startup_path = self._get_app_startup_path("nuke")
    ...
    def _prepare_softimage_launch(self):
        ...
        xsi_plugins = os.path.abspath(os.path.join(
            self._get_app_specific_path("softimage"), "startup", "Application", "Plugins"
        ))
    ...
    def _prepare_3dsmax_launch(self, app_args):
        """
        ...
        """
        startup_dir = self._get_app_startup_path("3dsmax")
    ...
    def _prepare_3dsmaxplus_launch(self, context, app_args, app_path):
        """
        ...
        """
        engine_path = sgtk.platform.get_engine_path("tk-3dsmaxplus", self._app.sgtk, context)
        if engine_path is None:
    ...
    def _prepare_houdini_launch(self, context):
        """
        Houdini specific pre-launch environment setup.
        """
        engine_path = sgtk.platform.get_engine_path("tk-houdini", self._app.sgtk, context)
        if engine_path is None:
        ...
        try:
            import tk_houdini
            tk_houdini.bootstrap.bootstrap(self._app.sgtk, context)
        except:
            self._app.log_exception("Error executing engine bootstrap script.")
            raise TankError("Error executing bootstrap script. Please see log for details.")

    def _prepare_flame_flare_launch(self, engine_name, context, app_path, app_args):
        """
    ...
    def _prepare_mari_launch(self, engine_name, context):
        """
    ...
    def _prepare_photoshop_launch(self, context):
        """
        Photoshop specific pre-launch environment setup.
        """
        engine_path = sgtk.platform.get_engine_path("tk-photoshop", self._app.sgtk, context)
        if engine_path is None:
        ...
        startup_path = self._get_app_startup_path("photoshop")
        sgtk.util.append_path_to_env_var("PYTHONPATH", startup_path)
    ...
    def _get_app_startup_path(self, app_name):
        return os.path.abspath(os.path.join(self._get_app_specific_path(app_name), "startup"))
