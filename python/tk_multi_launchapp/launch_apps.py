...
class BaseLauncher(object):
    ...
    def _init_launch_command(self, app_engine, app_icon, app_menu_name, app_path, app_args, version=None):
    ...
    def _launch_app(self, 
            ...
            version=None, context=None, file_to_open=None):
        """
        ...
        try:
            ...
            # the relevant engine.
            app_path = _apply_version_to_setting(app_path, version)
            app_args = _apply_version_to_setting(app_args, version)
            if app_engine:
            ...
            version_string = _get_clean_version_string(version)
            ...
            if return_code != 0:
                ...
                self._register_event_log(menu_name, app_engine, context, launch_cmd, version)
    ...
    def _register_event_log(self, menu_name, app_engine, ctx, command_executed, version=None):
        """
    ...
    def launch_app_from_entity(self, app_engine, app_path, app_args, version=None):
        """
        ...
        self._launch_app(app_engine, app_path, app_args, version)
...
class SingleConfigLauncher(BaseLauncher):
        ...
        if app_versions:
            for version in app_versions:
                self._init_launch_command(
                    self._app_engine, app_icon, self._app_menu_name, 
    ...
    def launch_from_path(self, path, version=None):
        ...
        self._launch_app(
            self._app_menu_name, self._app_engine, self._app_path,
            self._app_args, version, context, path
        )
...
class SoftwareEntityLauncher(BaseLauncher):
    ...
    def init_launch_commands(self):
        ...
        for sw_app in sw_entities:
            ...
            # in SG.
            app_versions = [ver.strip() for ver in (sw_app["sg_versions"] or []).split(",")]
            if app_versions:
                for app_version in app_versions:
                    if not app_version : 
                        continue
                    self._init_launch_command(
                        app_engine, app_icon, app_menu_name, app_path, app_args, version
                    )
            else:
                self._init_launch_command(
                    app_engine, app_icon, app_menu_name, app_path, app_args
                )
