...
def bootstrap_tank():
    ...
    try:
        ...
        log_msg("Shotgun: Could not import sgtk! Disabling for now: %s" % e)
        return
    ...
    try:
        ...
        log_msg("Shotgun: Could not create context! Shotgun Pipeline Toolkit will be disabled. Details: %s" % e)
        return
        
    try:    
        engine = tank.platform.start_engine(engine_name, context.tank, context)
    except Exception, e:
        log_msg("Shotgun: Could not start engine: %s" % e)
        return
    
    file_to_open = os.environ.get("TANK_FILE_TO_OPEN")
    if file_to_open:
        # finally open the file
        shotgunsupport.load_file(file_to_open)
