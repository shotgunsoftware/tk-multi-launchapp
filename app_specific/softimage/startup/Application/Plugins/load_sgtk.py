...
def XSIUnloadPlugin(in_reg):
    ...
    Application.LogMessage(
        str(in_reg.Name) + str(" has been unloaded."), constants.siVerbose
    )
    return true
...
def LoadShotgunPipelineToolkit_OnEvent(in_ctxt):
    ...
    if not "TANK_CONTEXT" in os.environ:
        Application.LogMessage(
            "Shotgun: Missing required environment variable TANK_CONTEXT.",
            constants.siError,
        )
        return
    ...
    try:
        ...
        Application.LogMessage(
            "Shotgun: Could not start engine: %s" % e, constants.siError
        )
        return
