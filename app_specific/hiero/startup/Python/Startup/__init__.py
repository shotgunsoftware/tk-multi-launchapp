...
def bootstrap_tank():
    ...
    try:
        ...
        hiero.core.error("Could not import Tank! Disabling Tank for now.")
        return
    ...
    try:
        tk = tank.Tank(project_root)
         ...
         hiero.core.error("The Tank API could not be initialized! Tank will be disabled. Details: %s" % e)
         return
    ...
    try:
        ...
        hiero.core.error("Could not determine the Tank Context! Disabling Tank for now. Details: %s" % e)
        return
    
    try:    
        engine = tank.platform.start_engine(engine_name, tk, ctx)
    except tank.TankEngineInitError, e:
        hiero.core.error("The Tank Engine could not start! Tank will be disabled. Details: %s" % e)
